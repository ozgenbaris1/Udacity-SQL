...
JOIN region r ON r.id = s.region_id
GROUP BY r.name
HAVING SUM(o.total_amt_usd) = (SELECT MAX(total_amt) FROM t1);
...
WITH 	t1 AS (	SELECT a.id, a.name, SUM(total_amt_usd)
				FROM accounts a
				JOIN orders o ON o.account_id = a.id
				GROUP BY 1,2
				ORDER BY 3 DESC
...
WITH 	t1 AS (	SELECT AVG(o.total_amt_usd) avg_amt
				FROM orders o
				),
		t2 AS (	SELECT a.name, AVG(o.total_amt_usd) avg_amt
				FROM orders o 
				JOIN accounts a ON a.id = o.account_id
				GROUP BY 1
